...
def ms2matlab(
    datacolname="DATA",
    msname="test1.MS",
    visfilename="bbsvis.mat",
    timeslot=0,
    modelcolname="MODEL_DATA",
    applyweights=True,
    channel=0,
):
    ...
    t = taql(
        "SELECT *, MAX(WEIGHT_SPECTRUM) as MW FROM $t0 WHERE TIME IN (SELECT DISTINCT TIME FROM $t0 OFFSET $timeslot LIMIT 1)"
    )
    ...
    for i in range(data.nrows()):
        ...
        for cor in range(4):
            if not flag[i][0][cor]:
                if applyweights:
                    V[2 * ant1 + cor / 2][2 * ant2 + cor % 2] = data[i][channel][
                        cor
                    ] * sqrt(weight[i][0][cor])
                    Vm[2 * ant1 + cor / 2][2 * ant2 + cor % 2] = model[i][channel][
                        cor
                    ] * sqrt(weight[i][0][cor])
                else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract data and model data from a MS, save in matlab format"
    )
    parser.add_argument(
        "msname",
        help="Name of the MS that contains data, model data and instrument table",
    )
    parser.add_argument("-d", "--datacolname", help="Data column name", default="DATA")
    parser.add_argument(
        "-m", "--modelcolname", help="Model column name", default="MODEL_DATA"
    )
    parser.add_argument(
        "-t", "--timeslot", help="Time slot from MS", type=int, default=0
    )
    parser.add_argument("-c", "--channel", help="Channel from MS", type=int, default=0)
    parser.add_argument(
        "-o", "--output", help="Save output in this output file", default="bbsvis.mat"
    )
    parser.add_argument(
        "-nw",
        "--no-weights",
        help="Do not apply weights, store them separately in the outputfile",
        action="store_true",
    )
    ...
    ms2matlab(
        msname=args.msname,
        datacolname=args.datacolname,
        timeslot=args.timeslot,
        modelcolname=args.modelcolname,
        applyweights=(not args.no_weights),
        visfilename=args.output,
