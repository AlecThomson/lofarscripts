    ...
    for timechunk in t.iter("TIMECHUNK"):
        ...
        for f1 in range(0, nfreq, freqstep):
            ...
            for pol in range(0, 4):
                outputdata[:, f1:f2, pol] = inputdata[:, f1:f2, pol] - np.average(
                    inputdata[:, f1:f2, pol][flags[:, f1:f2, pol]],
                    weights=weights[:, f1:f2, pol][flags[:, f1:f2, pol]],
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--freqstep", help="Number of channels to average", default=1, type=int
    )
    parser.add_argument(
        "-t", "--timestep", help="Number of timesteps to average", default=1, type=int
    )
    parser.add_argument(
        "-i", "--inputcol", help="Input column (default: DATA)", default="DATA"
    )
    parser.add_argument(
        "-o", "--outputcol", help="Output column (default: same as input column)"
    )
    ...
    if args.freqstep == 1 and args.timestep == 1 and False:
        ...
        subtractmean(
            args.ms, args.inputcol, args.outputcol, args.timestep, args.freqstep
        )
        print(
            "Subtracted (t=%d,f=%d) mean from %s, giving column %s."
            % (args.timestep, args.freqstep, args.inputcol, args.outputcol)
        )
